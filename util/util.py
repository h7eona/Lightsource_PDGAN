...
def input_process(img_path, image_size, device=torch.device("cuda"), mask=False):
    ...
    if mask:
        return (torch_image>torch.max(torch_image)/2).float()
    else:
...
def test_mask_process(mask_path):
    ...
    if mask_path.endswith((".png", ".jpg", ".jpeg", "JPG", "bmp")):
        ...
        mask_name_list = natsorted(
            [
                mask_name
                for mask_name in os.listdir(mask_path)
                if mask_name.endswith((".png", ".jpg", ".jpeg", ".JPG", ".bmp"))
            ]
        )

        mask_nums = len(mask_name_list)
        if mask_nums == 0:
            raise ValueError(f"The {mask_path} has no image, plead check your folder")
        else:
            for _, mask_path_each in enumerate(mask_name_list):
                mask_list.append(input_process(mask_path+'/'+mask_path_each, image_size=256, mask=True))
    return mask_list
