...
class PDGAN(BaseModel):
    ...
    def set_input(self, mask, gt):
        ...
        self.mask = transforms.Grayscale()(self.mask)
        # for i in range(256):
        #     for j in range(256):
        #         if self.mask[:, i, j] > 0.8:
        #             self.mask[:, i, j] = 1
        #         else:
        #             self.mask[:, i, j] = 0
        if self.mask.shape[0] == 1:
            self.mask = self.mask.repeat(3, 1, 1)
        self.inv_mask = (1 - self.mask).clamp(0, 1)
    ...
    def test_forward(self, image_num=2):
        ...
        out_dict = {}
        with torch.no_grad():
            b, c, h, w = self.gt.unsqueeze(0).size()
            # b, c, h, w = self.gt.size()
            fake_p_6, fake_p_5, fake_p_4, fake_p_3, fake_p_2, fake_p_1 = self.netEN(
                [self.input_img.unsqueeze(0), self.inv_mask.unsqueeze(0)]
            )
            self.pre_image = self.netDE(
                fake_p_1, fake_p_2, fake_p_3, fake_p_4, fake_p_5, fake_p_6
            ).detach()
            for i in range(image_num):
                ...
                out = self.netPDGAN(z, self.pre_image, self.inv_mask.unsqueeze(0))
                out_dict[f"out_{i}"] = out
        out_dict["input_image"] = self.input_img.unsqueeze(0)
        out_dict["mask"] = self.mask.unsqueeze(0)
        out_dict["ground_truth"] = self.gt.unsqueeze(0)
